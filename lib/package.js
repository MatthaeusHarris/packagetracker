...
module.exports = {
    ...
    getDeliveryEstimate: function(package) {
        switch(package.carrier.toLowerCase()) {
            case "usps":
                return "Delivery date not supported for USPS";
                break;
            case "ups":
                try {
                    if (package.lastResponse.Package.Activity[0].Status.StatusType.Description == "DELIVERED") {
                        return "Delivered."
                    }
                } catch(e) {
            ...
            case "fedex":
                try {
                    if (package.lastResponse.CompletedTrackDetails[0]) {
